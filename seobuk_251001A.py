...
HEYDEALER_ACCOUNTS = {
    "seobuk": "alskdj147!", # 실제 사용하는 ID와 비밀번호로 변경하세요.
    "inter77": "Seobuk2021**",
    "leeks21": "Dlrbtjd1366@"
}
...
def get_encar_location(driver):
    import re
    li_xpath = "//*[@id='detailSeller']//ul[starts-with(@class,'DetailSeller_info_seller__')]/li"
...
# 2) 주소+상호+이름 합치기(모달 금지)
def get_encar_seller_line(driver):
    addr = get_encar_location(driver)
    company_el = try_find(driver, By.XPATH, "//*[@id='detailSeller']//span[starts-with(@class,'DetailSeller_company__')]", timeout=1.0)
    name_el    = try_find(driver, By.XPATH, "//*[@id='detailSeller']//strong[starts-with(@class,'DetailSeller_name__')]", timeout=1.0)
    company = one_line(company_el.get_attribute("innerText")) if company_el else ""
...
def get_encar_title(driver, timeout=3):
    ...
    """
    X_H3 = '//h3[starts-with(@class,"DetailSummary_tit_car__")]'
...
def scrape_encar(driver, url, row_idx_hint):
    ...
    try:
        ...
        pass
    print(f"[Encar 모델명]: {out.get('name_ko', '추출 실패')}")

    try:
        ptxt = driver.find_element(By.CLASS_NAME, 'DetailLeadCase_point__vdG4b').text
        out["price"] = int(ptxt.replace(',', '')) * 10000
    except: out["price"] = None

    try:
        # ✅ WebDriverWait를 사용하여 번호판 요소가 나타날 때까지 기다립니다.
        plate_el = Wait(driver, 5).until(
            EC.presence_of_element_located((By.XPATH, '//*[@id="wrap"]/div/div[1]/div[1]/div[4]/div[1]/div/dl/dd[4]'))
        )
        out["plate"] = plate_el.text.strip()
    except TimeoutException:
        print("Warning: Encar plate element not found within 5 seconds.")
        out["plate"] = None
    except Exception as e:
    ...
    try:
        detail_btn = Wait(driver, 2.5).until(EC.presence_of_element_located((By.CLASS_NAME, "DetailSummary_btn_detail__msm-h")))
        click_js(driver, detail_btn); time.sleep(1)
        idx_map = {2:"year", 3:"mileage", 4:"engine", 5:"fuel_ko", 8:"color_ko"}
        for i,k in idx_map.items():
            el = try_find(driver, By.XPATH, f"(//span[contains(@class,'DetailSpec_txt__NGapF')])[{i}]")
            if not el: continue
            txt = el.text.strip()
            if k=="year":
...
# ==========================================================
# 1. SUPER PROJECTION 전용 변환 함수 (사용자 제공 인덱스 적용)
# ==========================================================
def to_super_projection_rows(records, start_row, user_name, seq_start=1):
    rows = []
    r = start_row
    for i, rec in enumerate(records, start=0):
        seq = seq_start + i
        ...
        row = [
            ...
            now_date(),              # E
            year,                    # F
            f'=googletranslate("{name_ko}","ko","en")' if name_ko else "", # G
            f'=googletranslate("{fuel_ko}","ko","en")' if fuel_ko else "", # H
            engine,                  # I
            mileage,                 # J
            plate,                   # K
            "",                      # L
            f'=COUNTIF($K$5:K{r}, K{r})', # M (차번호 중복체크)
            "-",                     # N
            "",                      # O
            f'=googletranslate("{color_ko}","ko","en")' if color_ko else "", # P
            site,                    # Q
            phone,                   # R
            location,                # S
            price if price is not None else "-", # T
            dealer_no,               # U
            f'=COUNTIF($U$5:U{r}, U{r})', # V (딜러 중복체크)
            cm_loc,                  # W
            cm_price,                # X
            "N/A",                   # Y: 운송비
            "N/A",                   # Z: 계산서O
            "N/A",                   # AA: 계산서X
            "N/A",                   # AB: 매도비
            f'=z{r}+aa{r}+ab{r}',                # AC: 총금액
            f'=((z{r}+ab{r})*0.915)+aa{r}+y{r}', # AD: NET COST
            f'=Ad{r}/Ag{r}',                   # AE: USD 판매원가
            "N/A" if site != "SEOBUK" else (rec.get("price_raw","-") or "-"), # AF
            f"=IFERROR(vlookup(E{r},'환율'!A:E,3,false),vlookup(E{r}+2,'환율'!A:E,false))", # AG
            f'=AF{r}-AE{r}',                   # AH: PROFIT
            f'=T{r}/$D$2',                     # AI: USD ENCAR
            f'=AF{r}-AI{r}',                   # AJ: USD Encar - Selling Price
            buyer,                             # AK
            f'=AF{r}/AG{r}*AN{r}',             # AL
            f'=T{r}/F{r}',                     # AM
            f"=iferror(vlookup(E{r},'환율'!A:E,5,false),vlookup(E{r}+2,'환율'!A:E,false))" # AN
        ]
        rows.append(row)
        r += 1
    return rows
...
def run_pipeline(list_pairs, user_name: str, headless: bool = False, hd_login_id: str = None):
    ...
    # A단계: 소스 긁기
    start_row_nuevo = read_existing_row_index()
    records = []
    plates = []
    hd_logged_in = False
    ...
    for idx, (url, buyer) in enumerate(list_pairs, start=0):
        row_hint = start_row_nuevo + len(records)  # 예상 행
        url = url.strip()
        lower_url = url.lower()
        buyer = buyer.strip()
    ...
    # D단계: 첫 번째 시트 (NUEVO PROJECTION) 기록
    rows_nuevo = to_sheet_rows(records, start_row_nuevo, user_name)
    flush_to_sheet(rows_nuevo, start_row_nuevo)
    print(f"NUEVO PROJECTION 시트 입력 완료 (Row: {start_row_nuevo})")

    # E단계: 두 번째 시트 (SUPER PROJECTION) 기록 (추가된 부분)
    try:
        SUPER_SHEET_ID = "1_vlIT35J3L9RnUiXW8m3Z3vnnWcOA9G1l34ak1PscCg"
        
        # 1. 새 스프레드시트 파일 열기 (인증 클라이언트 사용)
        # 기존 gc 객체가 이미 파일을 연 상태라면, 그 세션을 재사용하여 인증합니다.
        client = gspread.service_account(filename=Pp.Google_API()) 
        super_spreadsheet = client.open_by_key(SUPER_SHEET_ID)
        super_ws = super_spreadsheet.worksheet("SUPER PROJECTION")
        
        # 2. 시작 행 계산
        super_start_row = len(super_ws.get_all_values()) + 1
        
        # 3. 데이터 변환 및 업데이트
        rows_super = to_super_projection_rows(records, super_start_row, user_name)
        super_ws.update(f'A{super_start_row}', rows_super, value_input_option='USER_ENTERED')
        
        # --- 4. 테두리 및 기본 가운데 정렬 적용 (전체 범위) ---
        last_row = super_start_row + len(rows_super) - 1
        full_range = f'A{super_start_row}:AN{last_row}'
        ...
        super_ws.format(full_range, base_format)

        # --- 5. 특정 열(B, N, S, W, AK) 왼쪽 정렬 덮어쓰기 ---
        left_align_format = {'horizontalAlignment': 'LEFT'}
        
        # 지정하신 열들에 대해 루프를 돌며 개별 적용
        target_columns = ['B', 'N', 'S', 'W', 'AK']
        for col in target_columns:
            col_range = f'{col}{super_start_row}:{col}{last_row}'
            super_ws.format(col_range, left_align_format)

    except Exception as e:
        print(f"SUPER PROJECTION 시트 작업 실패: {e}")
